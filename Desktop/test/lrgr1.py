...
fig, ax = plt.subplots()
CS = ax.contour(X, Y, a, levels = np.arange(-1, 3, 0.1))
...
CS = ax.plot_surface(X, Y, a, cmap = cm.cividis)
ax.contour(X, Y, a, zdir='z', offset= -0.4, levels = np.arange(-1, 2, 0.1))
ax.clabel(CS, inline=True, fontsize=10)
ax.set_xlim([-0.1,6.1])
ax.set_ylim([-0.1,3.1])
...
ax = fig.gca(projection='3d')
CS = ax.plot_surface(X, Y, a, cmap = cm.cividis)
ax.contour(X, Y, a, zdir='z', offset=-4*10**(-19), levels = np.arange(-4*10**(-19),10**(-19), 0.2*10**(-19)))
ax.clabel(CS, inline=True, fontsize=10)
ax.set_xlim([-0.1,6.1])
ax.set_ylim([-0.1,3.1])
#fig.savefig('/Users/antonmnacakanov/Desktop/ToF/rgr1/3.11.pdf', dpi=1000)
fig.savefig('3.11.png', dpi=1000)
plt.close(fig)
