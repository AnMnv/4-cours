...
koef = [4, -2, 3, 5, -2, -4, 4, 5]
x = [1, 0, 0, 0, 0]
y = [0, 0, 0, 0]

for i in range(4):
    res = "y["+str(i)+"] = "
    y[i] =   4*(y[i-1] if (i-1)>=0 else 0)
    res += "4*"+str((y[i-1] if (i-1)>=0 else 0))
    y[i] += -2*(y[i-2] if (i-2)>=0 else 0)
    res += "-2*"+str((y[i-2] if (i-2)>=0 else 0))
    y[i] +=  4*(y[i-3] if (i-3)>=0 else 0)
    res += "+4*"+str((y[i-3] if (i-3)>=0 else 0))
    y[i] +=  5*(x[i] if (i)>=0 else 0)
    res += "+5*"+str((x[i] if (i)>=0 else 0))
    y[i] += -2*(x[i-1] if (i-1)>=0 else 0)
    res += "-2*"+str((x[i-1] if (i-1)>=0 else 0))
    y[i] += -4*(x[i-2] if (i-2)>=0 else 0)
    res += "-4*"+str((x[i-2] if (i-2)>=0 else 0))
    y[i] +=  4*(x[i-3] if (i-3)>=0 else 0)
    res += "+4*"+str((x[i-3] if (i-3) >=0 else 0))
